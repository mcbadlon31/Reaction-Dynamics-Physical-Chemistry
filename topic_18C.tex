...
% ===========================================================================
% Slide: Common Mistakes with TST
% ===========================================================================
\begin{frame}{Common Mistakes with TST}

\begin{alertblock}{⚠️ Watch Out For:}
\begin{enumerate}\itemsep=3pt
    \item \textbf{Confusing $E_a$ and $\Delta^\ddagger H^\circ$}
    \begin{itemize}\itemsep=1pt
        \item They differ by $RT$ (solution) or $2RT$ (gas, bimolecular)
        \item Relationship: $E_a = \Delta^\ddagger H^\circ + nRT$ where $n = 1$ or $2$
    \end{itemize}

    \item \textbf{Forgetting transmission coefficient $\kappa$}
    \begin{itemize}\itemsep=1pt
        \item Usually $\approx 1$ but not always!
        \item Tunneling: $\kappa > 1$ (quantum effects)
        \item Recrossing: $\kappa < 1$ (barrier recrossing)
    \end{itemize}

    \item \textbf{Using $\ln k$ instead of $\ln(k/T)$ for Eyring plot}
    \begin{itemize}\itemsep=1pt
        \item Arrhenius: plot $\ln k$ vs $1/T$ → slope $= -E_a/R$
        \item Eyring: plot $\ln(k/T)$ vs $1/T$ → slope $= -\Delta^\ddagger H^\circ/R$
    \end{itemize}
\end{enumerate}
\end{alertblock}
...
\begin{tikzpicture}[scale=0.8]
    ...
    % ZPE = (1/2)hν where ν ∝ 1/√m (for same force constant k)
    % So ZPE_D/ZPE_H = √(m_H/m_D) ≈ 1/√2 ≈ 0.707
...
\vspace{0.1cm}
\begin{enumerate}[a),topsep=0pt,itemsep=1.5pt]
...
% ===========================================================================
% Slide: Topic 18C Summary
% ===========================================================================
\begin{frame}{Topic 18C: Key Takeaways}

\textbf{Essential Equations:}
\begin{itemize}\itemsep=2pt
    \item \textbf{Eyring equation:} $k_r = \frac{k_BT}{h} e^{-\Delta^\ddagger G^\circ/RT}$
    \item \textbf{Thermodynamic form:} $k_r = \frac{k_BT}{h} e^{\Delta^\ddagger S^\circ/R} e^{-\Delta^\ddagger H^\circ/RT}$
    \item \textbf{Eyring plot:} $\ln(k/T)$ vs $1/T$ → slope $= -\Delta^\ddagger H^\circ/R$
    \item \textbf{KIE:} $\frac{k_H}{k_D} = e^{\Delta E_a/RT}$ (measures bond breaking)
\end{itemize}

\vspace{0.2cm}
\textbf{When to use TST:}
\begin{itemize}\itemsep=2pt
    \item[✓] Precise rate predictions
    \item[✓] Understanding entropy effects ($\Delta^\ddagger S^\circ$)
    \item[✓] Isotope effects \& tunneling
    \item[✓] Temperature dependence beyond Arrhenius
    \item[✗] \textbf{NOT} when barrier recrossing is significant ($\kappa \ll 1$)
\end{itemize}

\vspace{0.2cm}
\textbf{Key Insight:} TST connects thermodynamics to kinetics through the transition state!
